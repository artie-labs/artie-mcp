...
- `pipeline_list.status` is `draft` | `paused` | `transfer paused` | `running`. It is not lag. `pipeline_usage` is lag (seconds) and rows processed for a `from`/`to` window — not warehouse correctness, not slot size. There is no error-log or custom-monitor CRUD tool; those stay Dashboard.
